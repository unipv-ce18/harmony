...
class UploadContent(Resource):
    ...
    def post(self):
          ...
          content:
            application/json:
              ...
              examples:
                ...
                1: {summary: 'Audio type', value: {'category': 'song', mimetype: 'audio/flac', 'size': 1024}}
        responses:
          200:
            description: URL to upload and ID of the content
            content:
              application/json:
                example: ['url', {'presigned': 'data'}]
          400:
        ...
        if category != 'song':
            ...
            if category == 'artist' and result.creator != user_id:
                return {'message': 'No authorized to upload this content'}, HTTPStatus.UNAUTHORIZED

            if category == 'release' and result.artist.get(c.ARTIST_REF_CREATOR) != user_id:
                return {'message': 'No authorized to upload this content'}, HTTPStatus.UNAUTHORIZED
        ...
        url, params = {
            'image': get_images_post_policy,
            'audio': get_reference_songs_post_policy
        }.get(content_type)(conf, content_id, mimetype, size)
